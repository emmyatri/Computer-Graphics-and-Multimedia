 ...
 * Professor: Alan Shaw
 * Assignment #: Mod7, Assignment1, Lab1
 */
...
window.onload = function init() {
    ...
    document.getElementById("selObject").onchange =
        function (event) {
            ...
            let fileselected = event.target.value;
            if (fileselected == "") {
                objfile = "";
            }
    ...
    document.getElementById("showmesh").oninput =
        function (event) {
            showMesh = event.target.checked;
        };

    document.getElementById("removetexture").oninput =
        function (event) {
...
function bindLightingEntry(num) {
    let lightEntry = lightingArray[num];
    ...
    let shineVal = materialShininess;
...
function updateLightingArrayEntry(aVal, dVal, sVal, shineness) {
    let lightEntry = lightingArray.pop();
...
function configureTexture(img, filename) {
    ...
    let x = filename.indexOf(texdirname);
    let name = (x > -1) ? filename.substring(x) : filename;

    for (let m = 0; m < mtlfilename.length; ++m) {
        if (mtlfilename[m] == ("!" + name)) {
            mtlfilename[m] = name;

            for (let n = 0; n < mtlfilename.length; ++n) {
...
function readfile(objText) {
    ...
    for (let q = 0; q < lines.length; ++q) {
        ...
        if (line.length > 1 && line.charAt(0) == "v" && line.charAt(1) == " ") {
            ...
            line = line.substring(6).trim();
            let mm = line.indexOf(".mtl");
            if (mm > 0) {
                let mtlfile = line.substring(0, mm) + ".js";
...
function makemesh() {
    ...
    let y = document.getElementById("smoothshading");
    if (hasNorms) {
    ...
    if (hasTextures) {
        let found = false;
        for (let u = 0; u < mtlfilename.length; ++u) {
...
function render() {
    ...
    if (inread || imagesPending.length > 0 || mtlPending.length > 0 || texturePending.length > 0) {
        ...
        for (let h1 = 0; h1 < mtlindcode.length; ++h1) {
            if (skip.indexOf(h1) > -1)
                continue;

            let start = Number(mtlindval[h1]);
            let len = ((h1 == mtlindcode.length - 1) ?
                points.length : Number(mtlindval[h1 + 1])) - start;

            if (len > 0) {
                for (let i = start; i < start + len; i += 3)
                    gl.drawArrays(gl.LINE_LOOP, i, 3);
            }
        }
        if (mtlindcode.length == 0) {
    ...
    } else if (hasTextures && !hideTexture)
    {
        ...
        for (let h1 = 0; h1 < mtlindcode.length; ++h1) {
            ...
            let mtlFile = "";
            let mtlNum = -1;
            for (let h2 = 0; h2 < mtlfilecode.length; ++h2) {
